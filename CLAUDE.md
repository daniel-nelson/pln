...
**Sources, last verified 2026-09-17.** Re-fetch and re-read these before any work that changes a skill's size, structure, frontmatter or section order. They move: an earlier handoff into this repository cited only `platform.claude.com` and Anthropic engineering posts, by which time the authoritative spec had already moved to `agentskills.io`, and the agent working from that list reached conclusions the current text does not support. Treat a stale source list as the normal case rather than the exception, and re-stamp the date above when you have checked.
