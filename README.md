...
Same-host workers, including judgment-bearing planning, implementation, and review work, inherit the model you selected. pln does not classify capability from an alias, silently replace a lower-tier selection, or stop on an unfamiliar model name. `judgment` remains the semantic profile for decision-bearing work, and every worker runs at your session's effort. An optional `evidence_profile: economy` lane is limited to mechanically closed facts and citations; because it deliberately selects a different model, that exact host/model route remains version-qualified, and anything requiring interpretation escalates before it can recommend or decide.
