...
**Update check**: !`for d in "${CLAUDE_SKILL_DIR:-}" "$HOME/.agents/skills/pln" "$HOME/.claude/skills/pln" ".agents/skills/pln" ".claude/skills/pln"; do [ -n "$d" ] && [ -x "$d/bin/pln-update-check" ] && "$d/bin/pln-update-check" 2>/dev/null && exit 0; done`
