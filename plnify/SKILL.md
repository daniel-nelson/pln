...
- **Cut self-congratulatory adverbs.** Drop manner-adverbs that praise the outcome ("cleanly", "elegantly", "nicely", "seamlessly", "perfectly"): "That settles the session lifecycle", not "...cleanly". Adverbs that carry real meaning ("only", "roughly", "never") are fine.
- **Skip jargon and strained metaphors; use the plain word.** "load-bearing", "the rule that would bite", "moves the needle", "table stakes", "the real lever", "first-class" dress a plain idea in tech-bro costume. Say "important", "what everything depends on", "the rule that would work". Test: would you say the word to a friend who isn't an engineer? A word list won't keep up; watch for the reach-for-a-metaphor reflex.
