...
- **A user instruction to implement the feature under discussion, given during the interview, does not exit the interview.** "Fix it", "go do X now" — even said of the very bug you're mid-conversation about — is not authorization to leave Step 3, however it would be carried out: inline, or by a sub-agent, background task, or workflow. Acknowledge the request, record it in `PLAN.md` (as a decision, a new item, or a spinoff to run *after* the plan is adopted), and continue the interview. Emphasis, repetition, or specificity does not change this; naming the mechanism ("send off a sub-agent to do it") doesn't either — a request for feature work is a plan item regardless of who or what would carry it out. See Step 3 for what this rule does *not* cover: read-only research delegated to a sub-agent, and incidental capture of something outside the plan's own work (a stray bug, a feature request noted in another repo) — both are fine to act on immediately. The **only** thing that exits the interview early is the user explicitly abandoning the plan itself ("stop the plan", "forget the interview, just do it") — and even that never exits silently: recognizing the signal is not itself authorization to act on it. Confirm first, in these words or plainly equivalent ones: "Really exit `/pln` mode?" Only an explicit yes actually leaves Step 3; anything else — including no answer yet — means stay in the interview. This holds no matter how unambiguous the signal reads; unambiguous language is what makes it worth asking the question, not a reason to skip it. The binary "ask if unsure" escape hatch is reserved for actual ambiguity signals only: imperative-mood commands addressed to you to act *right now* ("go do X", "do it"), or explicit interview-exit language ("stop the plan", "forget the interview", "skip ahead") — and that ask-if-unsure question is separate from, and prior to, the exit confirmation: the first resolves whether this reads as an exit signal at all, the second confirms it before acting even once it does. **Naming a concrete action or outcome as a decision — including one that sounds executable, like "open a PR for X" — is never by itself ambiguous.** Litmus test: if the sentence would read naturally as a line inside an item's Decision write-up in `PLAN.md`, capture it silently and continue the interview. Do not ask. Every accepted plan item eventually becomes an executable action — that a decision names one is not a signal, it is the entire point of the interview.
<!-- pln:only claude -->
...
<!-- pln:endonly -->
- **Implementation runs through subagents; the orchestrator never does an item's work inline.** In the implementation phase (Step 5) the main session is a thin orchestrator: it reads `PLAN.md`, spawns one subagent per item, checks the file was updated, and moves on. It does not read code or edit files itself. Doing the work inline defeats the fresh-context guarantee and fills the orchestrator's context across the run. Spawning a sub-agent, background task, or workflow to do an item's actual feature work is **execution**, identical to doing it inline — it is never a way to make progress on the plan's own scope during Steps 1–4. Delegating *implementation* is a Step 5 mechanism only; delegating read-only research, or writing down an incidental discovery unrelated to the plan's own work, is fine at any time (see Step 3).
<!-- pln:only claude -->
...
The line between the two lanes is not "is this a write" or "is this delegated" — it's whether what's being asked for is the item's actual feature work. A note that says "Dream can't do X, someone should add it" is capture. Adding X to Dream is implementation, however small it looks, and it waits for Step 5 even if asked for explicitly and even if delegated to a background sub-agent that wouldn't block the interview — the ban is on doing the feature work now, not on background execution as a mechanism. When genuinely unsure which lane a request is in, the question to ask yourself is whether what's being asked would need its own tests-and-PR treatment; if yes, it's a plan item and gets captured, not started.
...
- **Building out the feature under discussion before the plan is adopted** — editing code, running a mutating command, or spawning a sub-agent/workflow to do it. Steps 1–4 are questions-only for the plan's own scope, and delegating the work is not a loophole. Before any state-changing tool call, stop and ask: is this the item's actual feature work — the thing that would need its own tests and PR — or is it read-only research / an incidental note unrelated to this plan (see Step 3, both of which are fine right now)? If it's the former and the plan isn't adopted yet (Step 4), the thing you want to do is a plan *item*, not an action you take now. This applies even when the user just asked for it, named the delegation mechanism themselves, or it targets a different repo.
