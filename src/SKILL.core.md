...
A finding that comes back from Step 3.5 ends in one of three places. It is **rejected** — the plan is not changed and the user is never told. It is **applied** — the plan's own prose is corrected before the user sees it. Or it is **flagged**, which leaves the plan alone and puts the finding at the approval gate for the user to act on. That call is made here, not by the reviewer.

**Flagged is the smallest of the three, not the default.** A flagged finding spends the user's attention, and it is worth spending only where the finding has more than one sensible outcome and which one wins is theirs. Everything else is either not a finding at all or is yours to fix. In the review this rule was rewritten from — a landing-page swap — ten findings reached the gate flagged; one of them turned on anything the user could have wanted either way, and their replies to the rest were "yes, add it", "a", "fix it", each of which is ratification, not a decision.

**The tell that this is miscalibrated: the user ratifies.** A flagged finding answered without discussion, or with the only sensible outcome, was a repair you should have made. Ratification clusters on omissions.

### Rejected

Some findings are the reviewer disagreeing with this skill rather than with the plan, and they stop here. A finding is rejected when it asks the plan to carry what "One filter, two surfaces" says the plan withholds — sequencing, internal code structure, exact invocations, error-handling shape, an unstated mechanism for something reversible — or when it is guardrail boilerplate ("add tests", "validate input"), or a preference between two things that both work. A reader handed a plan will always want more specificity in it; the plan withholds that on purpose, and forwarding the disagreement to the user asks them to adjudicate a rule they already have. Record nothing and say nothing.

### Applied

A finding is applied when all three of these hold. Failing any one of them flags it:

- **It is a false factual claim, a contradiction between two parts of the plan, or an omission whose completion is forced.** Those are the kinds where being right is checkable without knowing what the user wants. Anything turning on taste, risk appetite or domain knowledge is a judgment call and is flagged. The reviewer's own kind label is a claim, not a verdict — read the finding and decide the kind yourself. A stranger to the interview files a decision it disagrees with as a factual error routinely.

  **An omission is forced when exactly one completion is consistent with what the plan already decides.** The page needs three fonts and the plan vendors two; a workflow reads a flag in three places and the plan sets it in one; two documents describe a behavior being changed and the plan names one. There is nothing to weigh — the plan already says what it wants and the gap stops it delivering that, so the completion is the work, not a choice. Where two completions are both consistent with what is decided, picking is the judgment call and it is flagged. Where a project rule the user themselves wrote settles it — a `CLAUDE.md` that says the doc covering a changed behavior moves in the same PR — that is authority under What reaches the user, and quoting their own rule back to them for a ruling is the round trip that section exists to prevent.
...
6. **Hand both readers' results to one merge agent** — a fresh-context agent (see Spawning a fresh-context agent) that merges the findings and files them, so that reading every finding and deciding what survives never runs in this context. `/pln-pr`'s Step 3.1 delegates the same work for the same reason; this is that mechanism here. Its brief carries four things:
   ...
   - **The rules it files by**, which it reads whole from What a finding becomes in `{{SKILL_DIR}}/SKILL.md`: the reject-apply-flag test in that order, the quote opened and confirmed before anything is applied, and a finding landing on a decision the user made never applied whichever reader raised it. Say that flagging is the smallest of the three outcomes and that a finding with one sensible outcome is applied, not forwarded — a merge agent handed a list will otherwise pass most of it through, which is the failure this brief exists to prevent. It is a fresh agent, so it has none of this skill loaded — the path is how it gets them.
...
- Print **one numbered list of what was settled without the user**: the decide-and-disclose calls that clear all three tests in "What reaches the user", and — when Step 3.5 ran — the findings the review flagged rather than rejected or applied.

  **What earns a number is filtered before the list is built, not after.** A call that fails any of the three tests in What reaches the user is not listed — it is the work, and it lives in its item's section. That is the cut, and it is most of what used to fill this list.
