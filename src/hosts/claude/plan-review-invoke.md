Use one `Agent` call with `agentType: 'general-purpose'` (see Spawning a fresh-context agent). Its compact assignment points at `<plan-dir>/evidence/plan-review.brief.md` and names `<plan-dir>/evidence/plan-review.agent.out` as the findings path. The final message must be only `RESULT_FILE=<that path>`; never ask it to paste findings back.

The call's return value is the result pointer, the only thing that reaches this context.
