...
There is no phone-push channel on this host, and no tool to load for one. Codex's `notify` hook lives in the user's `~/.codex/config.toml` and fires on turn completion; a skill cannot address it at a particular wait, so it cannot provide targeted pln push notifications. The `notify_push` config key is ignored here.
