...
When the broad reviewer started beside the classifier, wait for it along with the other slots; the roster has returned only when it has too. After the roster returns, build the contract-first PR-merge prepared context from the raw paths and fixed completion metadata. Spawn the merge worker with that brief as its primary inventory rather than reconstructing a multi-pointer assignment. The merge worker verifies candidate, manifests, and artifacts before any reader can count.
