...
Read this file in full before the first fix decision or dispatch. Rebuild open clusters from `REVIEW.md`, not memory. **A change the user asks for during this phase becomes a cluster of its own**, recorded in `REVIEW.md` with its finding text and dispatched to a fresh worker like any other — not typed into the tree by the coordinator, whose job here is to brief, validate, commit and integrate. It is item work whoever asked, a one-line request included, and the coordinator that edits product files itself both loses the fresh-context guarantee and fills with the diffs and test output of every follow-up. **This phase carries its worker briefs inline, below — there is no separate contract file for a fix or a post-fix verifier.** The installed `src/workers/` directory holds contracts for other phases; searching it for this one's finds nothing and costs a round trip, which a real run spent. `execution-schedule.md` and `behavior-preservation.md` are the only contracts this phase names, and each is named where it applies. Keep the cursor here while fixes and their narrow verification/checkpoints complete.
...
Before editing, the worker validates the admitted baseline record and reruns the same admitted behavior suite on the actual pre-change source; afterward it reruns that suite under equivalent inputs and environment. Existing behavior-oriented tests are primary. Implementation-detail or post-only tests may supplement, never replace, this common boundary. Automatic deletion still requires private reachability, bounded consumer closure, and repository-native discovery, but those facts do not replace behavior proof. Missing, flaky, incomparable, indirect/dynamic, public, compatibility, persisted/stateful, consequential, or uncertain evidence retains the surface or becomes destructive/needs-a-decision. The full repository gauntlet remains the final regression floor in Step 7. No similarity score, raw-growth threshold, deletion target, or cleanup quota can authorize a simplifying mutation.
...
"Read the exact post-fix candidate. Reproduce the listed fixes and hunt for missed cross-cutting failures or regressions. Write only findings with exact citations and runnable reproduction/test evidence; label unsupported suspicions unverified. Give each finding a `reached_by`: the exact non-test `file:line` that can put the shipped system into the state your reproduction constructs, or `test-only`. Return only the assigned `RESULT_FILE` pointer."
...
**Repair progress, not a global round cap.** Adoption of `PR after implementation`, or direct invocation of `{{PLN_PR_CMD}}`, is standing authority to repair every verified, unambiguous, in-scope finding until assurance is clean and the PR is green. A new repair round is already-authorized repair work, not a new permission boundary. Global round count never blocks it.
