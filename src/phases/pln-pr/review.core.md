...
- **Return only a bounded envelope** to the coordinator: successful/failed reader attribution, header counts, acted-on cluster names, critical finding titles, the `pre_existing` titles, the exact tree fingerprint, and the staged candidate path/digest. Raw findings stay in evidence artifacts; canonical state changes only when the coordinator validates and publishes the candidate.

Validate the merge result through `bin/pln-read-envelope --root <plan-dir> --max-bytes 4096`; record its route in `routing.tsv`. A malformed merge gets one fresh judgment retry, then fails closed. Once its candidate is published, file each title in the merge envelope's `pre_existing` field once with `{{OUTPUT_ROOT}}/bin/pln-todo add`, `--source` naming this review and the finding: the base already had that defect, so it is filed and named rather than repaired here, and it is not an acted-on finding. If there are zero acted-on findings from at least one successful reviewer, note it and skip the fix pass: go to Step 6 (version/changelog) and then the Step 7 gauntlet.
