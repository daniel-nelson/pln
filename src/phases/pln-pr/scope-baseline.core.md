...
Persist status, reason, policy mode/hash, and the emitted bypass binding in `REVIEW.md` before review. `fresh` and `disabled` are silent. `due` is one disclosure and continues. Advisory `overdue` becomes a concrete follow-up; `unknown` continues with truthful attribution and never pretends the run is stale. `{{PLN_PR_CMD}}` never invokes `{{PLN_SIMPLIFY_CMD}}`.
