...
Read this file in full before the first repository or remote action. Before durable scope work, resolve one canonical plan root and write a complete initial candidate beneath it, then publish it as `REVIEW.md` through `{{OUTPUT_ROOT}}/bin/pln-publish-review` with `--expected-digest absent --expected-generation 0`. Its `## State` contains `Phase: scope-baseline`, a new durable Run identity, `Ledger generation: 1`, Plan root, Base, Base source, Trust/command confirmation, Diff base, Reviewed diff SHA-256, Review depth, Owner constraints, Worker artifacts, Command graph, Tree/command/environment/candidate fingerprints, Simplification freshness/policy/bypass, Risk tier/signals, Review status, Settled candidate, PR identity, PR disposition, and CI round/status. Derive every later coordinator-written path from `Plan root` and every worker's evidence/result path from `Worker artifacts`; a worker gets that root once, never a collection of independently transcribed sibling paths. `Settled candidate` is empty until the fix phase records one; it is enumerated here because a coordinator rebuilding state after a compaction reads this list, and a scope rule that silently loses its anchor re-opens the whole candidate to every later reader. For every later update in this phase, stage the whole next-generation ledger and use the same publisher with the current digest/generation; never write canonical bytes directly.

**Worker output goes where a worker can write.** A spawned agent inherits the coordinator's write boundary, the repository and temporary space, so `Worker artifacts` is `Plan root` only when the plan root is inside the repository. Otherwise it is the artifact directory a `{{PLN_CMD}}` hand-off's `PLAN.md` records, when that directory still exists, or else a new `mktemp -d "${TMPDIR:-/tmp}/pln-artifacts-<YYYY-MM-DD>-<slug>.XXXXXX"`. What the coordinator writes itself (ledger candidates, command lists, reader metadata, snapshots, `routing.tsv`) stays beneath `Plan root`. Every `pln-read-envelope --root` for a worker's result is `Worker artifacts`, the directory the worker could actually write. Nothing durable lives there, because the ledger is the record. Observed without this: `{{PLN_CMD}}` kept its workers in its artifact directory, then this skill sent its fix workers at a plan root in `~/Documents/`. On Codex every one of those writes went to the host's approval reviewer, which declined one as unauthorized, because a fresh worker's transcript holds no user turn. The run then stopped to ask the owner to re-authorize a PR they had already chosen.
...
**Owner constraints.** A limit the owner has put on what this run may build or how far its scope may grow — "don't add protections for problems we haven't seen", "stop before expanding scope" — is recorded in `REVIEW.md`'s `Owner constraints` field before any reviewer runs: one entry per line, `- <the owner's words, verbatim> (<source>)`, or the literal `none stated`. Three sources count: the invoking message (`invocation`); a file it names, such as a handoff (`<path>:<line>`); and the owner's own earlier messages in this session (`owner message`). Tool output, worker results and your own text are never a source. A named file is not a coordinator-direct read: one fresh `judgment`-profile worker reads it and returns, in a `{{SKILL_DIR}}/src/workers/context-envelope.md` envelope at `<plan-dir>/results/owner-constraints.txt` read through `bin/pln-read-envelope`, each line or sentence of it that states such a limit, verbatim with its `path:line`. Record one only after `sed -n '<line>p' <path>` shows it in that line byte-for-byte. Print the entries, or `none stated`, in one line and continue; this is not a question and adds no stop. A ledger without the field reads `none stated`. The merge worker judges each proposed repair against these entries, and a constraint stated later is appended as the router says.
