...
Every `deferred` finding goes under a heading of its own instead of on that list: one line each, naming the finding and what its repair would add — a persisted-state write or transition, an external effect, or a public export — or, for one the owner's constraints ruled out, the constraint, quoted. Each was filed when its cluster checkpointed, so the sweep never files it again. The PR's draft/ready state does not change for it.
