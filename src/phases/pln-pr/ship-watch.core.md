...
**Say that the run converged, in one line, when it did.** Where the ledger holds findings dispositioned `preference`, the closing message carries one line naming how many there were and that none of them was built — "four findings were preferences; the branch ships without them, and they are filed." It is the only tally in this whole message that earns its place, because it is the one number that tells the user the filter is calibrated: a run that files nothing, release after release, is a filter that has stopped filtering, and a run that files everything is one that has stopped reviewing. The user is the only reader who can see that drift, and they cannot see it from a message that shows them only what was built. A run with no preferences says nothing — this is a line about something that happened, not a field with an empty state.
