...
Give it the Plan root once; it derives the two artifact files, evidence path, and result path beneath it. Also give the persisted fingerprint, project root, routing attribution, and a 2048-byte budget:
...
Every `deferred` finding goes under a heading of its own instead of on that list: one line each, naming the finding and what its repair would add — a persisted-state write or transition, an external effect, or a public export. Each was filed when its cluster checkpointed, so the sweep never files it again. The PR's draft/ready state does not change for it.
...
**On a red required check:** capture logs file-first and have a fresh judgment worker classify the failure before editing: `infrastructure`, `flaky`, `permission`, or `code`. Infrastructure/flaky/permission failures do not authorize code changes; record evidence and retry/wait/escalate as appropriate. A code failure becomes a verified finding and a new candidate. Dispatch exactly one fresh CI fix cluster with its own `fix-ci-<round>-manifest.tsv`; never reuse a pre-PR worker or manifest. Its brief says it is a CI fix cluster, so a repair that would add new state, an external effect or a public export returns `BLOCKED:` rather than being deferred: a deferred code failure would leave the required check red until the three-round stop.
