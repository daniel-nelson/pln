...
4. Sweep the run's own record for outstanding work (see Follow-ups) before drafting anything. A run that never looks reports whatever it happens to remember. Then file it: every candidate that clears the follow-up bar goes into the to-do list here, one `{{OUTPUT_ROOT}}/bin/pln-todo add` per item with `--source` naming this run, before the message below is drafted rather than after it. Close the to-do list out in the same step: every item this run claimed gets the state its work actually reached, an item it finished is archived with the evidence that closed it, and `pln-todo stale`'s candidates are carried into the message below for the user to confirm — their answer is what archives one of those, and an unanswered candidate is named again at the next close.
5. Final message to the user: one or two sentences saying what changed and what's next, in plain words. This message is the complete answer on its own — no pointer to `PLAN.md` for the rest (see Style's "Ending a message"). If genuine follow-ups remain, list them per the follow-up bar below. **A delegated run repeats its short list here**, in full — the reversals, the findings the plan review flagged rather than repaired, the questions implementation had to assume its way past, each saying what was assumed and what a different answer would change, and the designs taken over a simpler existing-owner route the research named, each saying which route it was and why it lost. The user was asleep or away when that list first went out, so this is the reading it was written for. It is composed the same way it was the first time (see Delegated mode): no repairs, and every entry restating its item's number and title rather than pointing at one.
6. The to-do-location flow below is part of drafting that message rather than a step after it, and it is never gated on what the message turned out to list — filing already happened above, so the message is written from the to-do list. What the flow settles is what that message has to say: where the follow-ups went, and whether this project names a destination pln did not write to. Anything it asks or offers is a message of its own, separate from the ship ask Step 8 makes under `implement only`, an absent field, or a plan predating it. Under either PR-bearing value there is no ask to be separate from: the hand-off is an action, not a question.
...
- **False until Steps 6 and 7 are recorded done** — every ⏸ deferred item carries the user's revisit/push/drop answer, the dashboard's Verification section carries the gauntlet's per-command results and fingerprint hashes, the sweep's outcome is recorded in the plan, and the to-do-list close below has run. `Phase: finish-ship` is written before any of that happens, so without this bound a run resumed at that instant ships past its own verification. The wrap-up message leaves no durable mark of its own, so a turn that finds those writes in place and no sign the message went out sends it first and hands off in the same turn — the one thing the hand-off waits behind.
...
- **Prose about the hand-off comes after the hand-off, or not at all.** The one message allowed to precede it — the wrap-up above — says what changed and what is still open, and stops there. No sentence saying the run is about to hand off, and no notification line saying the PR is being opened. Step 5 already states this rule about the next dispatch, and it is here for the same reason: a future-tense sentence about your own next tool call reads as a sign-off, and the turn ends on it. Observed on a twelve-item run that had just passed its gauntlet — a push reading "opening the PR now", then a wrap-up ending "Handing off to `{{PLN_PR_CMD}}` now to review the branch and open the PR", then nothing for seventeen minutes, until the user asked "Are you still working, or did you stop?" The hand-off fired correctly on that turn, against unchanged state, so the trigger above was never what failed. The sentence was. `{{PLN_PR_CMD}}` announces itself in its own first lines.

**When the to-do-list close fires.** Step 7's item 4 is a position in this file the same way the hand-off was, and the same compaction, restart or interruption leaves it behind. It leaves no durable mark of its own, so a close that never ran reads exactly like one that did. Its trigger is a condition over durable state too, stated here because a run cannot reach the hand-off's lower bound above without it. While `Phase: finish-ship` stands:

- **False until the sweep has filed** — every candidate that cleared the follow-up bar is in the to-do list. That keeps file-first-then-draft intact and stops the close reading a record the sweep is still writing.
- **True from there until every id this run claimed carries an outcome** — `[x]` archived with the evidence that closed it, or `[-]` with what remains readable as sub-items in its detail file. The claimed set is the ids the dashboard's `## To-do items` section records as claimed. An id whose record there is a refusal — the holder it named, or the collision `check` reported — is not in that set and needs no outcome. An id carrying no recorded claim result at all has not been shown to be outside it either, so it holds the bound until the run records what the attempt returned or drops the id from the declaration; a claim can be live in the to-do list with nothing on the plan side saying so, and nothing else recovers it. Neither state is read off the absence of the other. A plan carrying no `## To-do items` section declared nothing and satisfies this bound.
- **Also true while any id the plan records as archived still has a live record** at `<todo-root>/items/<id>.md` held by this run — `claimed_by` and `claimed_in` both matching, or `claimed_by` alone where the record predates `claimed_in` and reads back empty. Archiving removes the record, so its presence there contradicts the plan's own prose about its own id. One file open per archived id; it never tries to confirm that an archive happened, and it is not a `stale` line, so the rule that nothing waits on those candidates is untouched.
- **While it holds, the close is the turn's first action and precedes the wrap-up message.** That is the opposite of the hand-off above, which waits behind that message. A divergence either bound reports goes to the user and holds the run short of `Phase: complete`; it is never noted and passed over. Where the to-do list cannot be resolved or written at all, the escape below stands as it is: say so, and list inline.
