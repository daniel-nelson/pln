...
**Always hand off a review depth.** `/pln-pr` asks for one itself when none arrives, and a plan that leaves it unset turns a run that was meant to go unattended into one waiting on an answer. Read it from the Ship field; where the field predates it and no ask was made, pass `full`.
