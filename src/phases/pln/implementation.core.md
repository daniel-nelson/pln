...
**A plan that takes queue items declares them here, before the first dispatch.** One `{{OUTPUT_ROOT}}/bin/pln-queue claim --id <id> --run <plan-dir name>` per item: the helper checks the collision and records the holder in the item's own record under one lock, so a second run in this tree reads the item as taken instead of taking it too. The claimed set is the only set this run may check off at the close. A refused claim names what it collided with — leave that item in the queue rather than working it anyway.
