...
**A change the user asks for mid-run becomes a node; it is not the coordinator's to type.** "Fix it", "make it consistent", "while you're there, also…" — a request that edits product files is item work whoever asked and however small it looks, and it goes the way every other item goes: give it an item number and a detail section in `PLAN.md`, append it to the manifest with its own write lease, dispatch one fresh worker, validate, checkpoint, integrate. The coordinator's part is what it always is — brief, validate, commit, integrate — and never the edit itself.

Two reasons, and the second is the one that bites. It is the same fresh-context guarantee every adopted item gets: a worker that reads only this change starts from the code rather than from an hour of accumulated conversation. And doing it inline fills the coordinator with the file contents, diffs and test output of every follow-up, which is exactly what the phase split exists to prevent — measured on a real run, a coordinator that edited twelve source files itself reached 198k tokens of a 258k window, and a two-word follow-up request then cost twenty minutes inside one inference. A one-line change is not an exception: it is the case where the inline edit looks cheapest and the context cost is identical.

The request also gets a durable record this way. Typed straight into the tree it exists only in the transcript, so a restarted session cannot tell it happened, and the run's own ledger disagrees with its commits.
### Auto-mode behavior

Auto mode applies only to **Step 5 (implementation phase)**. It does not stop immediately for a blocker: leave the blocked item's partial work in place, record the handoff/status in the manifest, and mark its dependents `waiting`. That partial work holds the working tree, so no other item is dispatched while it is unresolved, and nothing later in integration order crosses the blocker. Resolve blockers at the end, recompute readiness from the manifest, and continue. Never label a dependency wait as deferred work.
