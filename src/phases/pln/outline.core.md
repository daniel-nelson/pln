...
3. **Ask who wrote the source document, before anything reads it.** When the invocation names a path, or says part of the task is in a document, one question goes to the user before anything opens that document — before the research worker below is dispatched, and before this context reads a line of it: who wrote it, and are the solutions it proposes settled, or open to better ones? A feature request written up by another agent is a routine way for a run to start, and in the repository it reads exactly like an authority — a concrete source that already decides a choice — so everything it appears to settle gets settled without anyone being asked. The answer is what stops that; Step 3 is where it is applied.
   - **It reads nothing first. That is the whole point, and it is why the question is fast.** Do not open the file to judge whether the question is warranted: the prompt naming it is the trigger. Nothing infers authorship from a document's shape either — "this reads like an agent wrote it" guesses at a fact the user can give in one word.
   - **The trigger, at its edges.** A named path fires it, and so does a prompt saying part of the task is in a document without naming one. Several named documents are one question naming them all, not one question each. A URL is the same case, and it is still not fetched. `PLAN.md` and pln's own control state are not source documents — a continuing invocation resumes and does not ask, and neither does a run whose answer is already recorded in `PLAN.md`. A prompt that already says who wrote the document and whether its solutions are open has answered the question; take it and move on.
