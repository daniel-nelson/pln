...
- **Hang what was settled without the user under the items it belongs to.** There is no second list. The decide-and-disclose calls that clear The fork test, and — when Step 3.5 ran — the findings the review flagged rather than rejected or repaired, sit as sub-entries beneath their item's dashboard row, each addressed by that row's number plus its own letter: `1a`, `2b`. One number space covers the whole message, so nothing is numbered from 1 twice. This replaces printing them as a second list of their own: two lists both starting at 1, in different words for the same items, and the user could not tell whether the two entries numbered 1 were the same thing.
  ...
  **What earns an entry is filtered before the entries are built, not after.** Anything that fails The fork test gets none — it is the work, and it lives in its item's section. That is the cut, and it is most of what used to fill this space. Findings that all reopen the same decision are one entry, so the number of entries is the number of questions, not the number of defects.
