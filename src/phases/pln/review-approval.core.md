...
`false` or `no` means off, for every plan in every repository. Anything else — including absent, which is what an install that has never been told otherwise reads — means on. Off sends nothing and runs no reviewer. Usually it is a quiet authoritative opt-out; when the classification put this plan at the deepest setting, the gate must say plainly that critical plan assurance was skipped, and name the concrete trigger that earned that depth in place of the reader roles it has none of — "nothing read this plan, and it changes how money moves". Where the work could not be classified at all, that is what the sentence says: the deepest setting is the fail-closed default for work pln cannot classify, not a risk anyone found. `{{SKILL_DIR}}/bin/pln-config set plan_review false` turns it off, `true` back on.
...
Every reviewer sees the plan but never the interview transcript or rejected options. The merge worker alone reads raw findings, checks citations/evidence state, updates `PLAN.md`, and returns a bounded envelope; the one exception is a round `pln-assurance merge-skip` proves empty, whose record the coordinator writes (Step 3.5 step 5). A finding on a user-made decision is protected from repair. Empty or failed readers contribute nothing and are named accurately at the gate. The coordinator reads only the validated 4096-byte envelope; malformed merge output gets one fresh judgment retry, then fails closed.
...
5. First ask whether the merge has anything to read, without opening a single artifact:
   ...
   ```bash
   "$PLN_BIN/pln-assurance" merge-skip --roles <this round's roster roles, comma-separated> \
     --since "<plan-dir>/evidence/plan-review.brief.md" \
     --artifact <role>=<raw artifact path>   # once per reader that returned one
   ```

   The roles are every reader this round ran: on the first pass the second column of `pln-assurance roster`'s `SLOT` lines plus any additive peer from step 4, on a bounded round the readers you chose. Only `SKIP_MERGE=yes` skips the merge: every rostered reader wrote, after this round's brief, exactly the terminal `Nothing worth changing` line. Then write the round's record in `PLAN.md` yourself — the dashboard Plan review line with its scope, the readers that ran, and zero rejected, repaired and flagged — and, on a bounded round, remove the in-scope items' earlier Review findings and any Open questions entries that came from them, since this round's empty result replaces them. Re-read the edit. Any other output, a usage error included, keeps the merge.

   Otherwise spawn one fresh merge worker with `{{SKILL_DIR}}/src/workers/plan-review-merge.md`, the plan path, all raw artifact paths, actual-reader/role attribution, exact source fingerprint, whether applying is enabled, item scope, `evidence/plan-review-merge.md`, `results/plan-review-merge.txt`, and a 4096-byte budget. On a bounded round, these findings replace the in-scope items' earlier findings. The merge worker alone reads findings and edits `PLAN.md`; the skipped empty round above is the one record you write.
6. When the merge ran, validate its envelope through `bin/pln-read-envelope --root <plan-dir> --max-bytes 4096`. At least the broad reader must succeed; for R3, a failed role is a visible coverage failure rather than a clean plan. Never open raw findings in this context.
...
- **Hang what was settled without the user under the items it belongs to.** There is no second list. The decide-and-disclose calls that clear The fork test, and — when Step 3.5 ran — the findings the review flagged rather than rejected or repaired, sit as sub-entries beneath their item's dashboard row, each addressed by that row's number plus its own letter: `1a`, `2b`. One number space covers the whole message, so nothing is numbered from 1 twice. This replaces printing them as a second list of their own: two lists both starting at 1, in different words for the same items, and the user could not tell whether the two entries numbered 1 were the same thing.

  **What earns an entry is filtered before the entries are built, not after.** Anything that fails The fork test gets none — it is the work, and it lives in its item's section. That is the cut, and it is most of what used to fill this space. So does any finding the merge worker recorded `requested_by: unrequested`: it is informational, it never reaches this list, and no weight of consequence promotes it, because the plan never promised the capability it protects. Findings that all reopen the same decision are one entry, so the number of entries is the number of questions, not the number of defects.

  **A call already said in the flow still gets an entry**, marked as already-said where it hangs, one line each. It is tempting to collapse those into a clause with a count, since the user saw them once and one word would have stopped them — but that rests on the disclosure line having registered, and it lands mid-interview inside a message about something else. A line read past is not a decision ratified. Keep the entry, so the gate stays the one surface where everything decided without the user is visible and answerable by number. One number space across both kinds, so the user can reply "1a, 2b, P1" without saying which kind each is. Naming things the user reads wants a title with every number, and the row an entry hangs under is that title, printed one line above it — so the entry line does not repeat it, and a plan-wide entry, having no row above it, carries a short title of its own. Each entry is one line that opens with its kind — the marker below, never a label you invent for the topic:
  ...
  A prevention the plan took for an interleaving without asking (Step 3) is a ***decision*** entry that names the consequence it prevents, the surface (path, record, job or key) and the likelihood evidence. A user who strikes one accepts a risk, and it is written down beside the code, so walk the struck entry before recording anything. This phase has no transition back to the interview, so the walk happens here, as a question, one per turn. It needs both halves of one sentence: the consequence with how likely it is, and what the struck prevention adds. Where the plan does not record a half, dispatch a fresh `judgment` worker with `{{SKILL_DIR}}/src/workers/interview-research.md` in item mode for that surface, then ask. A likelihood or cost that research could not establish and the user does not know is recorded as unknown, with what research checked, and that ends the walk. Where research contradicts the likelihood or consequence the strike rests on, the question says so plainly, and the user still decides. Only then record a `Decision (user, selected)` pair whose option line is that entry's line, plus their words, so `{{PLN_PR_CMD}}` does not rebuild what they declined, and add to the item's acceptance criteria a comment placed at that surface, with its exact text, in the shape "<consequence, with its likelihood> is an acceptable tradeoff for avoiding <the prevention and what it adds>". It is composed from those recorded facts: a reason the user gave is restated in plain words, never quoted; where they gave none, nothing beyond the facts is added; an unknown half is said as unknown ("…, of unknown frequency, is…"); and it names no person and carries no chat wording or plan path. An adopt reply that carries a strike does not adopt until that walk is done.

  **Push back once when a strike accepts a harm it does not mention.** The struck entry named the consequence its prevention stops, so a strike in words that do not mention that consequence — a bare "2a" mentions nothing — gets, as the walk's first question, one statement of what it costs, with its evidence, and a question whether to keep the strike. The same holds for any answer in the walk that accepts a consequence the question named — lost or duplicated data, money, an irreversible effect or one outside the system — without mentioning it. Once only: the second reply is recorded as given. The `Decision (user, selected)` entry quotes the user's words from both replies verbatim, and the pushback appears in it only as a short agent-marked parenthesis, "(Pushback by the agent: …)", never as the user's words. For this one pushback these rules give way: not re-offering an option the user already answered ("The user has already answered it", below); the one-line echo, which carries the answer and nothing else; never arguing for an option after the list; and "the user ratifies", which reads a bare selector as a question that had one answer.
<!-- pln:only claude -->
...
- End with a five-way prompt, one option per line:
  ...
  d) implement only
  e) reopen an item (2) or a settled entry (2a, P1) / change something?
  ```
...
**When no review ran** — `plan_review` is off, the user skipped it, or the plan predates review — include no empty findings machinery. Below the deepest setting the entries under the rows are disclosed decisions only. At the deepest setting, add one clear warning that critical plan assurance was deliberately skipped, carrying the concrete trigger in place of the reader roles there are none of — "plan review is off, so nothing read this plan, and it changes how money moves", or, where the work could not be classified, that the deepest setting is the fail-closed default rather than a risk anyone found. Do not claim a reviewer or clean result.
