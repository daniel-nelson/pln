...
Once the user has authorized shipping, every newly verified, unambiguous, in-scope finding remains authorized repair work. Assurance progress is measured per behavioral defect, never by a global count of review rounds: a stable repair key follows the affected boundary and reproduction across moved citations, while a different verified reproduction is new work. `bin/pln-assurance repair-action` continues new findings and same-defect retries automatically; only three consecutive failed repairs of the same defect or a genuine decision, scope, destructive-action, or worker blocker returns control to the user.
