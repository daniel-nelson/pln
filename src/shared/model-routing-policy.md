...
Read `{{SKILL_DIR}}/bin/pln-config get evidence_profile` when an evidence task is dispatched. Missing and `inherit` keep the hosting model; `economy` automatically selects the host's economy model only for eligible evidence work. If that route is unavailable, fall back to inherited evidence work and attribute the fallback. Never advertise this setting during an interview; setup owns its one-time notice.
