...
**An item the user finished themselves, in this run, is not one of those.** When they say in this run that they did the work — their own words, about that item — and the run has verified it against the system rather than against the record, `pln-todo archive --disposition resolved-elsewhere` runs in the same turn, with their words and that verification as the evidence and the record's state left as it was. It is the completion counterpart of the dropped rule above and rests on the same ground: they settled it, and "say the word and I'll archive it" asks them to authorize a thing they did themselves. Observed: an item the user repaired mid-run, which the run then verified down to the audit entry and the whole delivery chain, was offered back at the close as a candidate anyway. Nothing else changes — the archive is still the one write a run makes outside its declared set, and the record still gains no `[x]`.
