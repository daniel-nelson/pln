...
**The layout, relative to the resolved to-do-list root.** The index is `<todo-root>/TO-DO.md`. Each live item has one detail file at `<todo-root>/items/<slug>.md`. A record that reaches a terminal state moves to `<todo-root>/archive/<YYYY-MM>/<slug>.md`, listed in that month's own `index.md`. `pln/` is only the default root's basename — never write a literal `pln/` into one of these paths, because the root is resolved and may sit outside the repository entirely.

**A list written before 1.59.0 carried different file names, and is renamed once, in place, the first time the helper touches it** — index, live directory and archive together. After that only the names above exist, and a pln older than 1.59.0 cannot read that list. The rename comes back as a `NOTE=`, so relay it like any other. A root found holding a complete older list *and* a complete newer one is refused with both named and nothing written: two lists are never merged, and neither is guessed at.

**The helper resolves the root; no phase document does.** `pln-todo` resolves the to-do list's location on every invocation, creates it when nothing is found, and reports whether the location question is still owed. A standalone `{{PLN_PR_CMD}}` run, a `{{PLN_CMD}}` run resumed after a restart, and a plan that predates this release all reach the same to-do list without having run any particular step first.

### The index line

The index is derived from the detail files rather than maintained by hand: `pln-todo list` rebuilds it whole, so a line that disagrees with its detail file is the line that is wrong.
...
- [-] ! ready · a cancelled booking never releases its held dates, so the room stays unbookable → `items/cancel-releases-held-dates.md`

## Refunds — money already taken and not yet given back

- [ ] decide · a guest charged twice gets one refund, and nobody has said which charge it clears → `items/double-charge-refund.md`
- [ ] blocked · the refund total on the receipt leaves out the cleaning fee → `items/receipt-refund-total.md`
```
...
- **Whether it can be picked up** — `ready`, `blocked`, `decide`, `proposed`, `dropped`. There is no `doing` and no `done`: `[x]` covers done, and what a run has in hand is the scope it declared when it started.

The three are orthogonal. Any item at any stage of completion can be urgent, and `[-] ready` and `[-] proposed` are both ordinary.

**`decide` and `proposed` are the seam, and getting it wrong is what makes a to-do list feel like a debt.** They are not two shades of one thing.

- `ready` — work anybody can pick up. Its shape is agreed; what is left is doing it.
- `blocked` — the same, waiting on something the item names.
- `decide` — **a run is stuck.** The work in front of it cannot go on until the user answers, and the answer is not one the run may pick. This is rare, and it is meant to be: pln's interview is the place a decision gets made, one question at a time, while the plan is still cheap to change. An item filed `decide` is a question that escaped it.
- `proposed` — **a run found something and the user has never seen it.** The shape is not agreed, so nobody should go implement it; nothing is blocked on it, so the user owes no answer. Most of what a sweep, a review or a sub-agent turns up is this.
- `dropped` — what a record wears between a decision against it and its archive, never a resting place.

Observed, on the store this vocabulary was written against: nine items filed `decide` and not one of them a run that was stuck. Six were a run's own discoveries — rule-surface sweeps, a simplification map, a plan-review flag, an interview's research step — which are `proposed`. Three were decisions **the user had already made**, to do the thing later or not now, recorded in the same word as an open question. Nothing was broken and nobody had done anything wrong; there was one word for three situations, so every close rendered settled matters and idle ideas back to the user as things they owed. That is what the split is for. `DECIDE_OPEN=` on every `pln-todo list` is how the count stays visible, and it is reported rather than capped: a ceiling would bound proposals, and the number is a symptom, not the disease.
...
Related items should read as one thing to decide about rather than as three separate asks. What makes a long to-do list hard is not its length; it is not knowing what to do next, and in what grouping.
...
The comparison is pairwise against the set of items a run has declared, never a partition of the whole to-do list. Items nobody has picked up need no lanes computed for them.

**A run is not refused its own path overlaps.** Where the item collided with is already held by this same run, from this same worktree, a `path` overlap is passed over rather than refused, and the claim says which overlap it passed — `EXEMPT <id> path <path>` beside the `COLLISION` lines. Inside one plan a path overlap between two items is already an ordering the scheduler computed before any wave ran, so refusing it a second time only stops a plan from claiming the items it is there to do: no plan whose two items share a file could otherwise mark its own headline item at close. The exemption is per pair and covers `path` alone. A shared `holds` token keeps refusing whoever asks, because the scheduler resolves leases as repository-relative paths and never reads a to-do-list record, so two same-run items holding one deploy slot would go concurrent with no interlock at all; an item declaring no `touches` keeps refusing too, since an unknown write set is not a path overlap and inherits nothing from that argument. A caller-named `--against` set is compared verbatim, with no exemption.

**A run is identified by its run string and the worktree it claimed from, together.** `claim` records both, in `claimed_by` and `claimed_in`, and both must match before an overlap is passed over. The run string is `<YYYY-MM-DD>-<slug>` and carries nothing repository-qualified, while a to-do-list root may be declared as an absolute or `~/`-rooted path that two unrelated repositories share — so on one day, with one slug, two runs would otherwise pass each other's overlaps with no scheduler edge between them. Within one worktree the run string is trusted as it stands. A same-run claim arriving from a different worktree is refused exactly like a foreign run's, and a record claimed before `claimed_in` existed matches no worktree and is exempted from nothing.

**A holder can be gone, and `claimed_in` is what says so.** A claim is the only assertion in this store with no expiry: it says somebody is writing those paths right now, and nothing but the holder giving it back ever ends it. So a run that never reaches its own close — a session closed, a worktree deleted, a machine rebooted — leaves a hold that outlives the work by any amount, and the next run is refused on behalf of a writer that stopped days ago. That is the ordinary case for anyone running several trees at once, not a corner. `claim`, `check` and `release` therefore ask whether the holder's worktree is still there, on the same rule the to-do list's own file lock has always used: act where the evidence says the holder is gone, never on a guess. An absent worktree is evidence — nothing is written from a directory that is not there — and a claim naming one drops out of the compared set, reported as `DEAD_HOLDER <id> <run> <worktree>` so a reader can tell an item nobody claimed from one whose claimant has gone. Everything else stands: a record predating `claimed_in` names no tree and proves nothing either way, and a tree still on disk counts as live whether or not any session is attached to it, because a run between turns and a run that died look identical from outside. A caller-named `--against` set is compared verbatim here too, liveness included — the caller named that set themselves.

**A claim either happens or it does not.** The write set a run declares with `--touches` lands before the collision check, because the check is what it is for — and a refused claim puts back what was there. The check and the record are taken under one lock precisely so a claim has two outcomes and not three; a refusal that permanently rewrote the item's write set is the third, and the run that wrote it has gone. What it leaves behind is a declaration nobody stands behind, and where it is narrower than what the item really writes, every later answer under-declares on its authority. A claim that succeeds keeps its declaration, which is the whole point of declaring at pickup.

**The holder is three fields and is never readable as part of one.** `claimed_by`, `claimed_at` and `claimed_in` are written together and cleared together, in a single rewrite of the record. Written one at a time they have windows between them, and a process that dies in one — a kill, a denied turn abandoned mid-flight — leaves a record carrying some of a holder and not the rest, with the to-do list's lock released on the way out and nothing to tell the next reader. Such a record reads as unheld to every tool here and as claimed to a person opening the file, and it names a date or a worktree for a run that does not hold it. `claimed_by` is the holder: a record with `claimed_at` or `claimed_in` and no `claimed_by` is a remnant, `stale` reports it as `holder-record-inconsistent` whatever its dates say, and `release` clears it. One `mark` is likewise one rewrite, for the same reason.

**`release` gives an item back.** It clears `claimed_by`, `claimed_at` and `claimed_in`, leaves everything else exactly as it is, and the record reads afterwards like one nobody has taken. A run releases its own claim; a claim whose worktree is gone is released by whoever needs it, on that evidence and with `RELEASE_REASON=holder-gone` saying so. A holder that is still there is never released out from under — that is `claim --steal`, where displacing a live run is a decision somebody makes and the record carries. Releasing something unheld is a quiet no-op, so a close that hands back every id it took does not have to remember which ones it actually got. Without this a claim had two ends only, the run's own close or somebody else's steal, and every other way a run can stop leaked the hold: there was no call that meant "I am not working this", and the one call shaped like it, `archive`, removes the record instead.
...
**What the answer covers, and what a second worktree actually shares.** Two trees meet in one to-do list more often than not, so the holder record is doing real work rather than covering a corner. The linked worktrees of one repository resolve their to-do list to the *shared* git directory — `git rev-parse --git-common-dir` is the same path from every one of them — so they see each other's items and each other's holds by default. Instructions naming one root do the same across unrelated repositories. What separates the claimants there is the pair `claimed_by` plus `claimed_in`, which is why the worktree sits beside the run string: a `<date>-<slug>` is not unique on its own. Only a tree whose to-do list resolved somewhere genuinely separate — its own project-root `pln/`, its own declared path — has a to-do list the other cannot see, and then neither to-do list can say anything about the other's work at all. That last case is a stated limit, not something the fields quietly close.

### Intake — how work reaches the to-do list

Work enters through four doors, and every one of them ends in the same call: `{{OUTPUT_ROOT}}/bin/pln-todo add`. The helper writes the detail file, rebuilds the index from it, and prints back the `INDEX_LINE` it wrote — so what was filed is something a later step can read rather than something the run remembers doing. Four hand-written appends would be four chances to write the line differently and four chances to skip it.
...
**2. The explicit user drop.** "File this for a dedicated session", in whatever words, said at any point in a run — mid-interview, mid-review, in the middle of a fix. It is filed in the turn it is said, and it neither pauses the phase it arrived in nor becomes a question. A drop that waits for the close is a drop that survives only in the conversation, which is the state this to-do list replaces.

**3. The mid-implementation discovery.** An item worker that finds work outside its item's scope returns it in its result; the **coordinator** files it at the checkpoint. Workers never write the to-do list themselves — the same rule that keeps them out of `PLAN.md` and the run manifest, and for the same reason: one writer, so two concurrent workers cannot both file the same discovery under two ids.
...
**A claimed item is closed out at the run's own close, and never at the merge.** The ids a run claimed are its own to finish: it declared them before it started, it did the work, and its gauntlet and CI are the evidence. So at whichever close hands that work back to the user, each claimed id is marked from what actually landed, and every one that reached `[x]` is archived `--disposition completed` in the same step — the PR and its green required checks as the evidence where a PR was opened, the commits where the run was implement-only. None of it is a question, and none of it waits for a merge: merging is the user's action, taken after the session has ended, so a run that holds an item open until it sees one leaves finished work reading as open work indefinitely. Observed: a run whose PR went green on all eighteen checks closed with both of its claimed ids still live, and archived them an hour later only when the user asked whether it was safe to exit — against the merge commit, having waited for it.

**Under a PR-bearing `Ship`, the mark belongs to `{{PLN_CMD}}`'s close and the archive to the `{{PLN_PR_CMD}}` close that follows it.** The work is on the branch when `{{PLN_CMD}}` closes, so the state it reached is knowable and is recorded then; the PR can still go red, and there is no inverse of `archive`. So `{{PLN_CMD}}` marks and hands off, and the PR close archives every `[x]` id once the required checks are green, as above. A PR left red, stuck or blocked archives nothing — its items keep the state they earned and stay live, which is what those states are for. Where no PR is being opened, `{{PLN_CMD}}`'s close is the one that hands the work back and it does both.

**A read of the to-do list never fails, and it is never satisfied by reading the index file by hand.** `pln-todo list` is a read to whoever calls it: only the refresh of the on-disk index needs the write lock, and everything it prints is derived from the detail files at that instant. When another run holds the lock it skips the refresh, says so, and renders the listing live — so a locked to-do list costs a stale index file and nothing else. The index does name its holders, so a caller that gives up on the helper and opens `TO-DO.md` itself still reads a claimed item as claimed — but it reads whatever the last refresh wrote, and a lock is exactly when that file is out of date, so the helper remains the answer to who holds what right now. A refusal says that in as many words, and a `NOTE=` names the holder whenever a lock changed what a call could do.

**A lock left behind by a run that is gone is cleared, once it can be proven gone.** The lock is a directory, so a run killed between taking it and releasing it — a sandbox timeout, an OOM, a hard interrupt — used to leave one that never expired and failed every later call against a holder that no longer existed. The holder now records its pid, host and time inside the lock, and a lock is broken only on evidence: the same host, and a pid that is not running. Another host, or an owner file that cannot be read, is waited on instead and then reported with the `rmdir` that clears it — guessing wrong here means two writers in one to-do list, which is the thing the lock exists to prevent.

**Marking is attributable, so a run that has been displaced cannot record a completion.** `mark` names the run doing it — `--run <name>` — and a record someone else holds refuses the write, naming the holder. `claim --steal` is the only thing that moves a holder and it does not tell the run it displaced, so without this a run whose item was taken mid-flight could still check it off, and its plan would carry a success the to-do list does not back. A run that really did the work takes the item back with `claim --steal <holder>` and then marks it; that is also how the run that stole it finds out the steal was contested. An unheld record is marked by anyone, as before, and `--run` is only needed where there is a holder to match.

**`[x]` is not a claim a run may make about work it did not verify.** An item checked off carries the commit or the evidence that closed it, in the record that is archived. `pln-todo archive --disposition completed` refuses a record that is not already `[x]`, so the marking and the evidence cannot come apart.
...
**The archive inherits the to-do-list root and is never asked about separately.** Wherever the to-do list resolved, `archive/` is beneath it — tracked where the to-do list is tracked, outside the working tree where the to-do list is. Excluding a tracked archive is the one case where archiving would be deletion from the repository's record: a tracked detail file moved into an ignored directory shows as a deletion, and the next commit makes it real, under a guarantee that nothing is deleted.

**A decision against an item is a disposition, not a status to park it in.** When the user decides in this run that an item will not be done — their own words, about that item — `pln-todo archive --disposition dropped` runs in the same turn, with those words as the evidence and the record's state left as it was rather than checked off. Their decision is the whole authority the archive needs, so there is nothing to confirm: asking permission to file it re-opens something they just settled. A run that instead sets `status: dropped` and leaves the item in the live list has filed the decision where the next close reads it back as open work, and the user is asked again about a thing they already ruled on — the one outcome the to-do list exists to prevent. Observed: an item the user had killed four days earlier, in the run that filed it, sat live and `dropped` until they were asked whether to archive it. `dropped` is what a record wears between the decision and the archive, and `stale`'s `dropped-not-archived` is a net for the run that died in between, not a resting place.

**"Not now" is an answer, and an answered item never stays `decide`.** The rule above covers the user deciding *against* an item. This is its twin, for the far commoner case: they decide the thing is worth doing and not here — its own session, after the release, once the other work lands. That is a decision, not a deferral of one, and the item leaves `decide` in the same turn it is given: to `ready` when the shape is now agreed, or `blocked` when it waits on something the item then names, with the user's own words as the record's evidence. Left at `decide`, it is filed exactly as an open question, and the next close asks them again about a thing they settled — which is the same failure as a live `dropped` record, one category over. Observed: three of the nine items above were this. One carried the user's own words in its `source` field — *asked for it as a dedicated session* — and was still rendered to them as an unanswered decision every close for eight days.

**Work the run found finished some other way is the user's call, every time.** `pln-todo stale` reports and never writes: an item whose record says `[x]`, one an earlier run left `dropped`, an abandoned claim, a claim whose holder's worktree is gone, a record carrying part of a holder, an item that has simply aged. Those are candidates, not findings. Every `STALE` line carries the item's claim after its holder, so the closing message names each candidate in the ordinary words it was filed under and never by its id alone; the user says which are actually finished or dropped, and only then does `archive` run — with their own words as the record's evidence, and with the record keeping the state it had rather than gaining an `[x]` nobody verified. A merged commit that names an item's id proves nothing on its own and archives nothing. A gone holder is the one line there that is a fact rather than a guess, and it still archives nothing: what it establishes is that nobody is working the item, not that the work is done.

**An item the user reports done in this run is not one of those.** When they tell the run that the item is finished — their own words, about that item, whether they did it or had it done elsewhere — and the run has verified it against the system rather than against the record, `pln-todo archive --disposition resolved-elsewhere` runs in the same turn, with their words and that verification as the evidence and the record's state left as it was. It is the completion counterpart of the dropped rule above and rests on the same ground: they settled it, and "say the word and I'll archive it" asks them to authorize what they already reported. Observed: a user who said mid-run *"I had Claude Cowork repair it. It did so, then confirmed that it is working"*, a run that went and verified that down to the audit entry and the whole delivery chain, and a close that offered the item back as a candidate anyway. Nothing else changes — the archive is still the one write a run makes outside its declared set, and the record still gains no `[x]`.
...
**File first, then draft.** The sweep gathers, the bar decides, `pln-todo add` files every candidate that clears it, and only then is the closing message written. The message is drafted from the to-do list; the to-do list is never assembled from the message. Reversed, the list is a statement of intent with nothing behind it to check it against, which is how those four went missing.
