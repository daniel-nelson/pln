...
5. **You work in the environment you were given, and you never build one.** Installing dependencies because none are installed, creating or resetting a database, copying an environment file, allocating a port, or running migrations to make a suite runnable is not your work — it is setup that was supposed to be done before you were dispatched, and doing it yourself is how a one-line change becomes an hour. If the environment you were handed cannot run what this item needs, that is a blocker: stop and say what is missing. Do not repair it.
   The line is whether the change is one this item owns and commits. Adding or upgrading a package the item genuinely needs edits `package.json` and the lockfile — tracked files, inside your lease, part of your diff — and the install that realizes it is part of that work. An install that changes no tracked file is rebuilding an environment, and is the case above. Where the tree's own lockfile has moved ahead of what is installed, say so and stop; re-syncing an environment to a tree belongs to whoever moved the tree.
6. Before claiming verification, re-read the project's completion instructions and reproduce any named environment condition this run did not already match. Run the assigned lightweight pre-commit checks. Fix failures before returning success.
7. Follow the assignment's commit owner exactly. When it says `worker`, commit a complete verified item only in the working tree you were given and within its explicit lease, with the required co-author trailer; never `--amend`, `--no-verify`, or broad staging. When it says `coordinator`, do not stage or commit. The host assignment owns which value applies; this shared contract does not infer it.
8. Never edit `PLAN.md`, `REVIEW.md`, the run manifest, or another coordinator-owned ledger. Put proposed item status, dead ends, artifacts, discoveries, and any cross-item note in the evidence/result artifacts; the coordinator validates them and performs the durable ledger update after the item checkpoint.
9. Capture memories when required by the project's rules.
