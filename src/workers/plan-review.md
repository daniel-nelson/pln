...
- For every finding, quote the evidence: `file:line` plus verbatim repository text, or the plan's verbatim sentence for an internal contradiction. An unverified claim stays explicitly unverified.
...
Report each finding with: item (or plan-wide), one sentence stating the defect, the exact quote it rests on, the proposed change, and kind (`false factual claim`, `plan contradiction`, or `judgment call`). End with `Nothing worth changing` when there are no findings.
