...
**A `judgment call` finding names what asked for the thing it protects.** The other two kinds are defects against what the plan already promises, and need nothing further. A judgment call is a different claim: that the plan should do something it does not currently do. A plan is not obliged to do everything that would be prudent, so this kind carries `requested_by` — the verbatim sentence, quoted from the plan as it stands, that asks for the behavior the finding protects. A recorded user decision, an item's intent, constraint, or acceptance criterion, or the owner goal under Pre-flight findings all qualify; so does a repository mandate, given as `file:line` plus its verbatim text. Where neither exists, the literal `unrequested`.

Answer it against the plan and the repository, never against your own proposed change, and quote rather than paraphrase — the merge worker opens what you cite and checks it. A sentence the finding itself wants added is not a request for it. `unrequested` is an ordinary answer and not a failed review: a real gap in something nobody asked for is still worth writing down, and this field is not a confidence score and does not rank the finding. What it settles is whether the finding may stop this plan or reach the user, and a capability the plan never promised does neither.

Report each finding with: item (or plan-wide), one sentence stating the defect, exact quote/citation, reproduction or named test, evidence state (`verified` or `unverified`), proposed change, kind (`false factual claim`, `plan contradiction`, or `judgment call`), and `requested_by` for every judgment call. End with `Nothing worth changing` when there are no findings.
